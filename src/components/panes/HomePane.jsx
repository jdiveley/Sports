...
export default function HomePane() {
  const { players, games, settings, sportConfig, updateSetting, loadSchedule, scheduleStatus } = useDfs();
      ...
      {sportConfig.hasSchedule ? (
        <div className="card">
          ...
          <div className="grid">
            ...
            <div className="c3"><label>{sportConfig.label} Week</label><input type="number" min="1" max="22" value={settings.week} onChange={e => updateSetting('week', e.target.value)} /></div>
        ...
        <div className="card">
          <h3>Get started</h3>
          <p className="note">{sportConfig.label} doesn't support weekly schedule import yet. Head to the DraftKings tab to pull a live slate or import a salary CSV directly.</p>
        </div>
      )}
      <div className="card">
        ...
        <div className="grid">
          ...
          <div className="c4 kpi"><b>🔗 Correlation</b><span>{sportConfig.stackMode === 'passcatcher' ? 'QB stacks and bring-backs' : 'Team/game correlation signals'}</span></div>
        </div>
      </div>
    </>
  );
